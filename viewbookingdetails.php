...
<body>
    <header class="header">
        ...
        <div class="containert">
            <h3 style="position:relative;left:360px;font-family:Times New Roman" , Times, serif;>Booking Details</h3>
            <form action="viewbookingdetails.php" method="POST">
                <input type="text" name="user_name" value="" placeholder="Enter username" style="position:relative;left:320px;top:20px;height: 40px;font-size: 16px; border-radius: 5px;
               padding-left: 10px;">&nbsp;&nbsp;
...
echo'<table class="table" style="border-radius:12px 12px 0 0;  
 ...
 <tr style="background: linear-gradient(135deg, #71b7e6, #9b59b6); ">
     ...
     <th style="padding:12px 15px;">Number of people</th>
...
if(!$con){
 ...
 $sql="select booking.b_id,booking.b_date,booking.travellig_date,booking.no_of_peoples,booking.c_userid,booking.p_id,package.destination,total_price,(no_of_peoples * total_price) as total from booking,package where booking.p_id=package.p_id;"; 
 $result=mysqli_query($con,$sql);
 while($data=mysqli_fetch_array($result))
 { 
  //$total=$data['no_of_peoples'] * $data['total_price'];
 echo'<tr>
